...
def process_rotated_object(rotated_object_np):
    ...
    return rotated_object_np, binary_mask

def rotate_and_inpaint(segmented_image, background_with_cutout, mask_array, original_image_path, model_id="kxic/stable-zero123", pose=[0.0, -72.0, 0.0], output_dir="new_logs"):
    ...
    # Process the rotated object to remove the background and get the binary mask
    processed_object_np, binary_mask = process_rotated_object(rotated_object_np)

    # Calculate the bounding box around the black void in the mask_array
    x, y, w, h = cv2.boundingRect(mask_array.astype(np.uint8))

    # Resize the rotated object to fit within the blackout area
    resized_object = cv2.resize(processed_object_np, (w, h), interpolation=cv2.INTER_AREA)

    # Adjust the position to ensure the object is resting on the correct surface
    y_offset = y + (h - resized_object.shape[0])

    # Ensure the object is placed correctly within the blackout void area
    ...
    roi[object_mask] = resized_object[object_mask]  # Place the rotated object in the region of interest

    # Save the merged image
    final_image_path = os.path.join(output_dir, "final_merged_image_no_inpainting.jpg")
